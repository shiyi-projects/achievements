...
Future<void> appBootstrap(Ref ref) async {
  ...
  // 同步引擎首拉。结果通过 SyncStatusController 透出给 UI。
  ref.read(syncStatusControllerProvider.notifier).set(SyncStatus.syncing);
  final status = await ref.read(syncEngineProvider).pullOnce();
  ref.read(syncStatusControllerProvider.notifier).set(status);
}
